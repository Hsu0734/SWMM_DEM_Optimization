...
i = 1
for index in range(0, 499):  # range的结束值是不包含的，所以这里是9而不是8
    ...
    for step in sim:
        ...
        runoff_solution.append(runoff)

    fig = plt.subplot(1, 1, 1)
    fig.set_ylabel("Total Runoff volume (mm)")  # label标签字号
    fig.set_xlabel("Time")
    fig.plot(time_stamps, runoff_solution, label=f"solution{i}")
    i = i + 1
...
system_routing = SystemStats(sim)
time_stamps = []
original_runoff_solution = []

for step in sim:
    time_stamps.append(sim.current_time)
    runoff_stats = system_routing.runoff_stats
    runoff = runoff_stats.__getitem__('runoff')
    original_runoff_solution.append(runoff)


fig.plot(time_stamps, original_runoff_solution, label=f"default situation")
fig.grid("xy")
